...
>
> 项目创建：2026-08 ｜ 状态：**视觉线闭环（docs/177-220）+ 通用认知纲领与证据阶梯（docs/221-253）：L5 临界点（docs/232）、组合抽象 COMPOSABLE（docs/235）、流式动态（docs/241）、半真实线九格（docs/243-251）、泛化 L2 完整证明 + L3 跨域机制级（docs/246-251）、L4 组合泛化预注册已冻结（docs/253）**。
...
## 2026-08 新增：通用认知纲领与泛化证明（docs/231-253）

> 视觉线之上，本会话把"感知证据线"延伸为**通用认知假说的可测支线**：残差驱动、
> 利害绑定、结构从行为涌现。泛化按 docs/247 六层逐层证明，每格预注册+防循环+留出+守卫。

| 块 | 内容 | 文档 |
|---|---|---|
| 纲领 | 通用认知假说 H1-H3/F1-F5；维度图（十维交集/8 族 30 维/核心批/组分-催生-产物-保护四角色）；泛化六层规范 | docs/231/238-242/247 |
| 证据阶梯 | L5 临界点（结构涌现于中间态）；组合抽象 COMPOSABLE（上下文槽位）；二阶残差探针（否定→长程翻案）；流式动态（A1 时间，DYNAMIC_PASS） | docs/232/235/237/241 |
| 半真实线 | 机制迁移 → 分档证伪 → 软匹配 → 跨半径工作点（TENSION_RESOLVED）→ L3 跨域 → 快慢双原型（人眼机制，MECH_PASS）→ 配额退役（QUOTA_RETIRED） | docs/243-251 |
| 泛化 | L2 分布内偏移完整证明（留出+防循环+工作点余量）；L3 跨域机制级自足（稳定/结构/gist 正确性） | docs/246-248/251 |
| 下一步 | L4 组合泛化接真实流（上下文槽位 → 快慢双原型，预注册已冻结） | docs/253 |
